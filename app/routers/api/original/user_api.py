...
async def add_watch_list_item(data: Annotated[SeriesWatchListModel, Form()], service: SQLiteService = Depends(get_db_service)):
    ...
    item = getSeriesWatchListItem(data.series_id, service.engine) # FIXME

    if not item:
        addSeriesWatchListItem(data.series_id, service.engine) # FIXME
...
async def remove_watch_list_item(series_id: str, service: SQLiteService = Depends(get_db_service)):
    ...
    item = getSeriesWatchListItem(series_id, service.engine)

    if item:
        deleteSeriesWatchListItem(item.id, service.engine)
...
async def add_book_wish_list_item(data: Annotated[BookWishListModel, Form()], service: SQLiteService = Depends(get_db_service)):
    ...
    item = getBookWishListItem(data.book_id, service.engine) # FIXME

    if not item:
        addBookWishListItem(data.book_id, service.engine) # FIXME
...
async def remove_book_wish_list_item(book_id: str, service: SQLiteService = Depends(get_db_service)):
    ...
    item = getBookWishListItem(book_id, service.engine) # FIXME

    if item:
        deleteBookWishListItem(item.id, service.engine) # FIXME
