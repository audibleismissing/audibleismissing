...
def getAuthor(engine:create_engine, search_string):
    """Get author from db"""
    with Session(engine) as session:
        statement = select(AuthorsTable).where(or_(AuthorsTable.name == search_string, AuthorsTable.authorAsin == search_string, AuthorsTable.id == search_string))
...
def getBookAuthors(engine:create_engine, book_id) -> list:
    ...
    with Session(engine) as session:
        ...
        author_mappings_query = select(AuthorsMappingsTable).where(AuthorsMappingsTable.bookId == book_id)
        author_mappings_table = session.exec(author_mappings_query).all()

        authors = []
        for author_mappings_row in author_mappings_table:
            author_query = select(AuthorsTable).where(AuthorsTable.id == author_mappings_row.authorId)
