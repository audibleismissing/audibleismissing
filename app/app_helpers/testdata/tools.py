...
from app.db_models.db_helpers import resetAllData
...
def exportDbToJson(engine):
    ...
    with Session(engine) as session:
        # Export main tables
        data['authors'] = [row.model_dump() for row in session.exec(select(AuthorsTable)).all()]
        data['books'] = [row.model_dump() for row in session.exec(select(BooksTable)).all()]
        data['genres'] = [row.model_dump() for row in session.exec(select(GenresTable)).all()]
        data['narrators'] = [row.model_dump() for row in session.exec(select(NarratorsTable)).all()]
        data['series'] = [row.model_dump() for row in session.exec(select(SeriesTable)).all()]

        # Export mapping tables
        data['authormappings'] = [row.model_dump() for row in session.exec(select(AuthorsMappingsTable)).all()]
        data['genremappings'] = [row.model_dump() for row in session.exec(select(GenreMappingsTable)).all()]
        data['narratormappings'] = [row.model_dump() for row in session.exec(select(NarratorMappingsTable)).all()]
        data['seriesmappings'] = [row.model_dump() for row in session.exec(select(SeriesMappingsTable)).all()]

        # Export wishlist tables
        data['bookwishlist'] = [row.model_dump() for row in session.exec(select(BookWishListTable)).all()]
        data['serieswatchlist'] = [row.model_dump() for row in session.exec(select(SeriesWatchListTable)).all()]

    with open(json_file, 'w') as f:
        json.dump(data, f, indent=2, default=str)  # default=str to handle Decimal and datetime
...
def importJsonToDb(engine):
    ...
    with Session(engine) as session:
        ...
        # session.exec(select(SeriesWatchListTable).delete())
        # session.exec(select(BookWishListTable).delete())
        # session.exec(select(SeriesMappingsTable).delete())
        # session.exec(select(NarratorMappingsTable).delete())
        # session.exec(select(GenreMappingsTable).delete())
        # session.exec(select(AuthorsMappingsTable).delete())
        # session.exec(select(BooksTable).delete())
        # session.exec(select(SeriesTable).delete())
        # session.exec(select(NarratorsTable).delete())
        # session.exec(select(GenresTable).delete())
        # session.exec(select(AuthorsTable).delete())
