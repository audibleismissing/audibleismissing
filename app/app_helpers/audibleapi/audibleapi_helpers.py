...
def returnBookObj(api_book: dict, isSeries: bool) -> Book:
    ...
    book.audibleOverallAvgRating = round(
        api_book["rating"]["overall_distribution"].setdefault("average_rating", None), 2
    )
    book.audiblePerformanceAvgRating = round(
        api_book["rating"]["performance_distribution"].setdefault(
            "average_rating", None
        ),
        2,
    )
    book.audibleStoryAvgRating = round(
        api_book["rating"]["story_distribution"].setdefault("average_rating", None), 2
    )
